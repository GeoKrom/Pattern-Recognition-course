...
def KMeansFunc(K, dataset, rightdataset):
    kmeans = KMeans(n_clusters=K, init = "random",  n_init=20)
    kmeans.fit(dataset)
    contingency_matrix = metrics.cluster.contingency_matrix(rightdataset, kmeans.labels_)
    ...
    for i in range(K): #Gia kathe K
        ...
        for j in range(len(dataset)): #Gia kathe paradeigma
            label = kmeans.labels_[j] #Krata to label tou paradeigmatos sumfwna me ton kmeans
...
def eigenDecomposition(AffMatrix, plot = False):
    ...
    if plot:
        ...
        plt.scatter(np.arange(np.round(eigenvalues, decimals = 2)), np.round((eigenvalues), decimals = 2))
        plt.grid()
        plt.show()
